...
/* base64 로 받은 이미지를 그대로 올립니다 (텍스트 변환을 거치면 파일이 깨져요) */
async function writeBinary(path, base64, message, sha) {
  const enc = encodeURIComponent(path);
  return gh(`/repos/${REPO}/contents/${enc}`, {
    method: 'PUT',
    body: JSON.stringify({
...
exports.handler = async function (event) {
  ...
  try {
    ...
    */
    ...
    if (action === 'save') {
      ...
      */
      ...
      if (wantsObject) {
        ...
        return json(400, { error: '저장할 내용이 배열이 아닙니다' });
      }
    ...
    /* --- 쓰레드 광고 이미지 올리기 --- */
    /*
      쓰레드는 "인터넷에서 볼 수 있는 사진 주소"를 요구합니다.
      그래서 사진을 저장소에 넣어 사이트 주소로 만들어 줍니다.
        images/threads/<이름>.jpg  →  https://toureplus.co.kr/images/threads/<이름>.jpg
    */
    if (action === 'upload') {
      const name = String(req.name || '').trim();
      const b64  = String(req.data || '');

      if (!/^[a-z0-9][a-z0-9._-]{0,60}\.(jpg|jpeg|png|webp)$/i.test(name)) {
        return json(400, {
          error: '파일 이름이 올바르지 않습니다',
          detail: '영문·숫자·하이픈만 쓰고 .jpg .jpeg .png .webp 중 하나로 끝나야 합니다.',
        });
      }
      if (!b64) return json(400, { error: '이미지 내용이 비어 있습니다' });

      /* base64 는 원본보다 약 1.37배 커집니다. 8MB 원본까지 받습니다. */
      if (b64.length > 11 * 1024 * 1024) {
        return json(413, { error: '사진이 너무 큽니다', detail: '8MB 이하로 줄여서 올려 주세요.' });
      }

      const path = IMG_DIR + name.toLowerCase();
      let sha = null;
      try { sha = (await readFile(path)).sha; }   /* 같은 이름이면 교체 */
      catch (e) { if (e.status !== 404) throw e; }

      await writeBinary(path, b64, '[관리자] 쓰레드 광고 사진 ' + (sha ? '교체' : '추가') + ' — ' + name, sha);

      return json(200, {
        ok: true,
        path: path,
        replaced: !!sha,
        message: '사진을 올렸습니다. 1~2분 뒤부터 쓰레드에 쓸 수 있습니다.',
      });
    }
